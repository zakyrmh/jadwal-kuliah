...
function getCountdown(targetDay, targetTime) {
    ...
    if (dayDiff < 0) {
        ...
        if (targetHours < now.getHours() || (targetHours === now.getHours() && targetMinutes <= now.getMinutes())) {
            dayDiff += 7;
        }
    }
    ...
    const targetDate = new Date(now.getFullYear(), now.getMonth(), now.getDate() + dayDiff, hours, minutes);
...
function displaySchedule(data) {
    ...
    data.schedule.sort((a, b) => getCountdownMilliseconds(a.hari, a.jamMulai) - getCountdownMilliseconds(b.hari, b.jamMulai));
    ...
    data.schedule.forEach((item, index) => {
        ...
        htmlContent += `
            <div class="bg-[#31363F] rounded-md shadow-md p-3 w-full max-w-[500px]">
                <h3 class="text-[#EEEEEE] text-center font-semibold mb-2">${item.mataKuliah}</h3>
                <div class="flex items-center justify-between">
                    ...
                    <div class="font-medium">
                        <span class="text-[#EEEEEE]" id="${countdownId}">${getCountdown(item.hari, item.jamMulai)}</span>
                    </div>
                </div>
            </div>
        `;
